...
export const getters = {
  projetos: (state, getters, rootState) => {
    // in componentes always get projetos from here
...
export const actions = {
  ...
  removeUsuario: ({ commit, state, rootState }, { idUsuario }) => {
    const toRemove = state.usuariosProjeto
      ...
      .map(relation =>
        UsuariosProjetos.delete(relation.id, rootState.usuario.token)
      )

    commit('IS_FETCHING', true)
    Promise.all(toRemove)
      .then(responses => {
        const updatedUsuarioProjeto = state.usuariosProjeto.filter(
          relation => relation.usuario !== idUsuario
        )
  ...
  reset_usuariosProjetos: ({ commit }) => {
    commit('RESET_USUARIOS_PROJETOS')
  },
  reset: ({ commit }) => {
    commit('RESET')
  }
...
export const mutations = {
  ...
  SET_PROJETOS: (state, { values }) => {
    state.projetos = values
  },
  SET_USUARIOS_PROJETOS: (state, { values }) => {
    state.usuariosProjeto = values
  },
